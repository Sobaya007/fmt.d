...
if (is(Int : int))
{
    ...
    this(Int2)(Int2 x) @nogc if (is(Int2 : int))
    {
        this.x = x;
    }
...
version (unittest) enum Init(T) = T.init;

import std : isBasicType;
interface I;
interface I2(T) : I if (isBasicType!T)
{
    T get();
}

template Po(T, alias S) if (is(T : int[]))
{
